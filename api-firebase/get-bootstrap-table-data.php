...
if (isset($_GET['table']) && $_GET['table'] == 'users') {
    ...
    $sql = "SELECT COUNT(u.id) as total FROM `users` u LEFT JOIN user_addresses ua on u.id=ua.user_id LEFT JOIN pincodes p on p.id=ua.pincode_id LEFT JOIN area a on a.id=ua.area_id " . $where;
    $db->sql($sql);
    $res = $db->getResult();
    foreach ($res as $row)
        $total = $row['total'];

    $sql = "SELECT u.*,a.name as area_name,p.pincode as pincode,p.city as city,ua.pincode_id FROM `users` u LEFT JOIN user_addresses ua on u.id=ua.user_id LEFT JOIN pincodes p on p.id=ua.pincode_id LEFT JOIN area a on a.id=ua.area_id " . $where . "  GROUP by u.name ORDER BY " . $sort . " " . $order . " LIMIT " . $offset . ", " . $limit;
    $db->sql($sql);
    $res = $db->getResult();
    $bulkData = $rows = $tempRow = array();
    $bulkData['total'] = $total;
...
if (isset($_GET['table']) && $_GET['table'] == 'area') {
    ...
    $sql = "SELECT COUNT(a.id) as total FROM `area` a JOIN pincodes c ON a.pincode_id=c.id " . $where;
    $db->sql($sql);
    $res = $db->getResult();
    foreach ($res as $row)
        $total = $row['total'];

    $sql = "SELECT a.*,c.pincode as pincode,c.city as city_name FROM `area` a join pincodes c ON a.pincode_id=c.id $where ORDER BY $sort $order LIMIT $offset,$limit";
    $db->sql($sql);
    $res = $db->getResult();
...
if (isset($_GET['table']) && $_GET['table'] == 'sales_reports') {
    ...
    //    echo  $sql = "select oi.*,o.mobile,o.order_note,o.total,p.name as product_name ,o.delivery_charge,o.discount,o.promo_code,o.promo_discount,o.wallet_balance,o.final_total,o.payment_method,o.address,o.delivery_time,p.name as name, u.name as uname,v.measurement, (SELECT short_code FROM unit un where un.id=v.measurement_unit_id)as mesurement_unit_name,oi.status as order_status from `order_items` oi left join product_variant v on oi.product_variant_id=v.id left join products p on p.id=v.product_id left join users u ON u.id=oi.user_id left join orders o ON o.id=oi.order_id where oi.order_id=o.id  $where ORDER BY $sort $order LIMIT $offset , $limit";
    $sql = "select oi.id,o.total,oi.seller_id,o.user_id,o.mobile,p.name as product_name,o.final_total,o.address, u.name as uname, oi.status as order_status,DATE(oi.date_added) as date_added from `order_items` oi left join product_variant v on oi.product_variant_id=v.id left join products p on p.id=v.product_id left join users u ON u.id=oi.user_id left join orders o ON o.id=oi.order_id where oi.order_id=o.id and oi.active_status='delivered'  $where ORDER BY $sort $order LIMIT $offset , $limit";
    $db->sql($sql);
    $res = $db->getResult();
    $bulkData = array();
    $bulkData['total'] = $total;
...
if (isset($_GET['table']) && $_GET['table'] == 'seller') {
    ...
    foreach ($res as $row) {
        ...
        // $operate .= ' <a class="btn-xs btn-danger" href="delete-seller.php?id=' . $row['id'] . '"><i class="fa fa-trash-o"></i>Delete</a>';
        // if ($row['status'] == 7) {
        //     $operate .= "<a class='btn btn-xs btn-warning restore-seller' data-id='" . $row['id'] . "' title='Restore'>  <i class='fa fa-eye'></i>Restore </a>";
        // } else {
        $operate .= ' <a class="btn-xs btn-danger" href="remove-seller.php?id=' . $row['id'] . '"><i class="fa fa-trash-o"></i>Remove</a>';
        // $operate .= "<a class='btn btn-xs btn-danger remove-seller' data-id='" . $row['id'] . "' title='Remove'>  <i class='fa fa-eye'></i>Remove </a>";
        // }
...
if (isset($_GET['table']) && $_GET['table'] == 'pincodes') {
    ...
    if (isset($_GET['search']) && !empty($_GET['search'])) {
        $search = $db->escapeString($fn->xss_clean($_GET['search']));
        $where .= " Where `id` like '%" . $search . "%' OR `pincode` like '%" . $search . "%' OR `city` like '%" . $search . "%' ";
    }
