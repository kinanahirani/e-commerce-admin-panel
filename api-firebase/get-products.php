...
if (isset($_POST['get_all_products']) && $_POST['get_all_products'] == 1) {
    ...
    */
    ...
    foreach ($res as $row) {
        ...
        if(!empty($pincode) ){
            $res_pincode = $fn->get_data($column=['city','pincode'],"id=".$pincode , 'pincodes');
            $row['deliverable_area'] = $res_pincode[0]['city'] . " - ".$res_pincode[0]['pincode'];
        }else{
...
if (isset($_POST['get_products_offline']) && $_POST['get_products_offline'] == 1 && isset($_POST['product_ids']) && !empty($_POST['product_ids'])) {
    ...
    */
    ...
    $sql = "SELECT  count(pv.id) as total FROM `products` p JOIN `seller`s ON s.id=p.seller_id WHERE p.is_approved = 1 AND p.status = 1 AND s.status = 1 AND p.id IN ($product_ids) ";
    $db->sql($sql);
    $total = $db->getResult();

    $sql = "SELECT p.*,s.name as seller_name,s.status as seller_status FROM `products` p JOIN `seller`s ON s.id=p.seller_id WHERE p.is_approved = 1 AND p.status = 1 AND s.status = 1 AND p.id IN ($product_ids)";
    $db->sql($sql);
    $res = $db->getResult();
    $product = array();
    $i = 0;
...
if (isset($_POST['get_variants_offline']) && $_POST['get_variants_offline'] == 1 && isset($_POST['variant_ids']) && !empty($_POST['variant_ids'])) {
    ...
    */
    ...
    $sql = "SELECT  count(pv.id) as total FROM product_variant pv JOIN products p ON p.id=pv.product_id JOIN seller s ON s.id=p.seller_id where pv.id IN ($variant_ids) and p.is_approved = 1 AND p.status = 1 AND s.status = 1 ";
    $db->sql($sql);
    $total = $db->getResult();
    ...
    foreach ($res as $row) {
        $sql = "select pv.*,p.*,s.name as seller_name,s.status as seller_status,pv.measurement,(select short_code from unit u where u.id=pv.measurement_unit_id) as unit from product_variant pv left join products p on p.id=pv.product_id JOIN seller s ON s.id=p.seller_id where pv.id=" . $row['id'];
        $db->sql($sql);
...
if (isset($_POST['type']) && $_POST['type'] == 'products-search') {
    ...
    */
    ...
    $user_id = (isset($_POST['user_id']) && is_numeric($_POST['user_id'])) ? $db->escapeString($fn->xss_clean($_POST['user_id'])) : "";
    $sql = "SELECT COUNT(p.id) as total FROM `products`p JOIN `seller` s ON s.id=p.seller_id WHERE p.is_approved = 1 AND p.status = 1 AND s.status = 1 ";
    $db->sql($sql);
    $res = $db->getResult();
    foreach ($res as $row) {
        $total = $row['total'];
