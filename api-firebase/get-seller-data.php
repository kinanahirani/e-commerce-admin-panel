...
if (isset($_POST['get_seller_data']) && $_POST['get_seller_data'] == 1) {
    ...
    */
    ...
    if (isset($_POST['pincode_id']) && $_POST['pincode_id'] != "" && is_numeric($_POST['pincode_id'])) {
        ...
        $where .=  " AND pincode_id= $pincode_id";
    }
    $response = array();
    $sql_query = "SELECT * FROM `seller` WHERE status = 1 $where";
    $db->sql($sql_query);
    $result = $db->getResult();
    $rows = array();
    $tempRow = array();
    ...
    foreach ($result as $row) {
        $res_pincode = $fn->get_data($columns = ['city', 'pincode'], "id=" . $row['pincode_id'], 'pincodes');
