...
if (isset($_POST['place_order']) && isset($_POST['user_id']) && !empty($_POST['product_variant_id']) && !empty($_POST['place_order'])) {
    ...
    // if (empty($_POST['total']) || $_POST['delivery_charge'] == "" || $_POST['final_total'] != "" || empty($_POST['payment_method']) || empty($_POST['address_id'])  || empty($_POST['delivery_time']) || $_POST['quantity'] == "") {
    //     $response['error'] = true;
...
if (isset($_POST['update_order_status']) && isset($_POST['order_item_id']) && isset($_POST['order_id'])) {
    ...
    if (!empty($res)) {
        ...
        if ($postStatus == 'delivered') {
            $sql = "SELECT oi.delivery_boy_id,o.final_total,o.total FROM orders o join order_items oi on oi.order_id=o.id WHERE oi.id=" . $order_item_id;
            $db->sql($sql);
