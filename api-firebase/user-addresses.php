...
if ((isset($_POST['add_address'])) && ($_POST['add_address'] == 1)) {
    ...
    */
    ...
    if (!empty($user_id) && !empty($type) && !empty($name) && !empty($mobile) && !empty($address) && !empty($landmark) && !empty($area_id) && !empty($pincode_id) && !empty($state) && !empty($country)) {
        if ($is_default == 1) {
        ...
        if ($db->insert('user_addresses', $data)) {
            ...
            $sql = "select ua.*,u.name as user_name,u.mobile as mobile,a.name as area_name,p.pincode as pincode,p.city as city,a.minimum_free_delivery_order_amount as minimum_free_delivery_order_amount,a.delivery_charges as delivery_charges from user_addresses ua LEFT JOIN area a ON a.id=ua.area_id LEFT JOIN pincodes p ON p.id=ua.pincode_id LEFT JOIN users u ON u.id=ua.user_id where ua.user_id= " . $user_id . " ORDER BY is_default DESC";
            $db->sql($sql);
            $res1 = $db->getResult();
    ...
    if (!empty($id) && !empty($user_id)) {
        ...
        if ($fn->is_address_exists($id)) {
            ...
            if ($db->update('user_addresses', $data, 'id=' . $id)) {
                $d_charges = $fn->get_data($columns = ['minimum_free_delivery_order_amount', 'delivery_charges','name'], 'id=' . $area_id, 'area');
                $pincodes = $fn->get_data($columns = ['pincode', 'city'], 'id=' . $pincode_id, 'pincodes');
    ...
    */
    ...
    if (!empty($user_id)) {
        if ($fn->is_address_exists($id = "", $user_id)) {
            ...
            $sql = "select ua.*,u.name as user_name,a.name as area_name,p.pincode as pincode,p.city as city,a.minimum_free_delivery_order_amount as minimum_free_delivery_order_amount,a.delivery_charges as delivery_charges from user_addresses ua LEFT JOIN area a ON a.id=ua.area_id LEFT JOIN pincodes p ON p.id=ua.pincode_id LEFT JOIN users u ON u.id=ua.user_id where ua.user_id= " . $user_id . " ORDER BY is_default DESC";
            $db->sql($sql);
            $res = $db->getResult();
