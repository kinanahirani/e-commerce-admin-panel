...
if (isset($_POST['btnAdd'])) {
    ...
    if ($permissions['locations']['create'] == 1) {
        ...
        if (empty($pincode_ID) && $pincode_ID != "" && $pincode_ID == 0 ) {
        ...
        if ($TOTAL == 0) {
            ...
            if (!empty($area_name) && !empty($pincode_ID) && !empty($delivery_charges) && !empty($minimum_free_delivery_order_amount)) {
                $sql_query = "INSERT INTO area (name, pincode_id,delivery_charges,minimum_free_delivery_order_amount)	VALUES('$area_name', '$pincode_ID',$delivery_charges,$minimum_free_delivery_order_amount)";
                $db->sql($sql_query);
                $result = $db->getResult();
                if (!empty($result)) {
                    $result = 0;
