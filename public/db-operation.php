...
if (isset($_POST['update_return_request']) && $_POST['update_return_request'] == 1) {
    ...
    if ($return_status == 1) {
        ...
        echo "total" . $total = ($res_oi[0]['discounted_price'] == 0) ? ($res_oi[0]['price'] * $res_oi[0]['quantity']) + ($res_oi[0]['tax_amount'] * $res_oi[0]['quantity'])  : ($res_oi[0]['discounted_price'] * $res_oi[0]['quantity'])  + ($res_oi[0]['tax_amount'] * $res_oi[0]['quantity']);
...
if (isset($_POST['add_seller']) && $_POST['add_seller'] == 1) {
    ...
    $sql = "INSERT INTO `seller`(`name`, `store_name`, `email`, `mobile`, `password`, `store_url`, `logo`, `store_description`, `street`, `pincode_id`, `state`, `account_number`, `bank_ifsc_code`, `account_name`, `bank_name`, `commission`,`status`,`categories`,`require_products_approval`,`national_identity_card`,`address_proof`,`pan_number`,`tax_name`,`tax_number`,`customer_privacy`,`latitude`,`longitude`) VALUES ('$name','$store_name','$email', '$mobile', '$password','$store_url' ,'$filename', '$store_description', '$street',$pincode_id,'$state','$account_number','$bank_ifsc_code','$account_name','$bank_name','$commission','$status','$cat_ids','$require_products_approval','$national_id_card','$address_proof','$pan_number','$tax_name','$tax_number','$customer_privacy','$latitude','$longitude')";
    if ($db->sql($sql)) {
