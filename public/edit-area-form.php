...
$sql_query = "SELECT id, pincode,city FROM pincodes ORDER BY id ASC";
$db->sql($sql_query);
...
if (isset($_POST['btnEdit'])) {
	...
	if ($permissions['locations']['update'] == 1) {
		...
		if (!empty($area_name) && !empty($pincode_ID) && !empty($delivery_charges) && !empty($minimum_free_delivery_order_amount)) {
			$sql_query = "UPDATE area SET name = '$area_name' , pincode_id = $pincode_ID, `minimum_free_delivery_order_amount`=$minimum_free_delivery_order_amount,`delivery_charges`=$delivery_charges WHERE id = $ID";
			$db->sql($sql_query);
						...
						<div class="form-group">
							...
							<select name="pincode_ID" class="form-control">
								<?php foreach ($res_city as $row) {
									...
									if ($row['id'] == $res_area[0]['pincode_id']) {

								?>
										<option value="<?php echo $row['id']; ?>" selected="<?php echo $res_area[0]['pincode_id']; ?>"><?php echo $row['pincode']." ".$row['city']; ?></option>
										<?php } else {
										if ($permissions['locations']['read'] == 1) { ?>
											<option value="<?php echo $row['id']; ?>"><?php echo $row['pincode']." ".$row['city'];  ?></option>
								<?php }
									}
								} ?>
							</select>
