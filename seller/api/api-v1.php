...
if (isset($_POST['get_categories']) && !empty($_POST['get_categories'] == 1)) {
    ...
    if (!empty($res)) {
        ...
        foreach ($res as $r) {
            ...
            $childs = $db->getResult();
            $childs[0] = array('id' => "0", 'category_id' => "0", 'name' => "Select SubCategory", 'slug' => "", 'subtitle' => "", 'image' => "");
            if (!empty($childs)) {
...
if (isset($_POST['get_products']) && !empty($_POST['get_products'] == 1)) {
    ...
    $sort = (isset($_POST['sort']) && !empty($fn->xss_clean($_POST['sort']))) ? $db->escapeString($fn->xss_clean($_POST['sort'])) : 'p.id';
    ...
    $sql = "SELECT count(p.id) as total FROM products p JOIN seller s ON s.id=p.seller_id $join where p.seller_id = $seller_id AND p.status=1 AND p.is_approved = 1 AND s.status = 1" . $where;
    $db->sql($sql);
    $total = $db->getResult();
...
if (isset($_POST['get_financial_statistics']) && !empty($_POST['get_financial_statistics'] == 1)) {
    ...
    $total_orders = $fn->rows_count('order_items', '*', "seller_id = $seller_id");
    ...
    $sql1 = "SELECT SUM(sub_total) AS total_sale,DATE(date_added) AS order_date FROM order_items WHERE YEAR(date_added) = '$year' AND DATE(date_added)<'$curdate' AND seller_id=$seller_id GROUP BY DATE(date_added) ORDER BY DATE(date_added) DESC  LIMIT 0,7";
    $db->sql($sql1);
...
if (isset($_POST['get_orders']) && !empty($_POST['get_orders'])) {
    ...
    foreach ($res as $row) {
        ...
        $res_seller = $fn->get_data($columns = ['name', 'mobile', 'latitude', 'longitude', 'state', 'street', 'pincode_id'], "id=" . $res[$i]['seller_id'], 'seller');
        $res_pincode = $fn->get_data($columns = ['city', 'pincode'], "id=" . $res_seller[0]['pincode_id'], 'pincodes');
        $state = (!empty($res_seller[0]['state'])) ? $res_seller[0]['state'] . ", " : "";
        $street = (!empty($res_seller[0]['street'])) ? $res_seller[0]['street'] . ", " : "";
        $pincode = (!empty($res_seller[0]['pincode_id'])) ? $res_pincode[0]['city'] . " - " . $res_pincode[0]['pincode'] : "";
...
if (isset($_POST['update_order_status']) && !empty($_POST['update_order_status'] == 1)) {
    ...
    if (empty($_POST['order_id']) || empty($_POST['status']) || empty($_POST['seller_id'])) {
        $response['error'] = true;
    ...
    $response = $fn->update_order_status($id, $order_item_id, $postStatus, $delivery_boy_id = 0);
    print_r($response);
}
...
/* 10.add_products
    ...
    serve_for:Available/Sold Out
*/
if (isset($_POST['add_products']) && !empty($_POST['add_products'] == 1)) {
    ...
    $res_msg .= (empty($_POST['delivery_places']) || $_POST['delivery_places'] == "") ? "delivery_places," : "";
    ...
    if ($db->sql($sql)) {
        ...
        $res_data = array(
            ...
            "loose_stock_unit_id" => $loose_stock_unit_id,
            "type" => $type,
...
11.update_products
    ...
    stock_unit_id:1
*/
if (isset($_POST['update_products']) && !empty($_POST['update_products'] == 1)) {
    if (ALLOW_MODIFICATION == 0 && !defined(ALLOW_MODIFICATION)) {
        $response['error'] = true;
        $response['message'] = "This operation is not allowed in demo panel!.!";
        print_r(json_encode($response));
    ...
    $res_data = array(
        "id" => $id,
        "product_variant_id" => $product_variant_id1,
        "name" => $name,
        "seller_id" => $seller_id,
        "tax_id" => $tax_id,
        "subcategory_id" => $subcategory_id,
...
if (isset($_POST['get_customers']) && !empty($_POST['get_customers'])) {
   ...
   */
    ...
    if (!empty($res)) {
        ...
        $sql = "SELECT DISTINCT u.*,a.name as area_name,p.pincode as pincode,p.city as city,ua.pincode_id FROM `users` u LEFT JOIN user_addresses ua on u.id=ua.user_id LEFT JOIN pincodes p on p.id=ua.pincode_id LEFT JOIN area a on a.id=ua.area_id " . $where . " ORDER BY " . $sort . " " . $order . " LIMIT " . $offset . ", " . $limit;
        $db->sql($sql);
        $res = $db->getResult();
        $rows = array();
        $tempRow = array();
