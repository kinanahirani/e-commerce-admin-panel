...
$sql = "SELECT oi.*,o.final_total as payable_total,oi.id as order_item_id,p.*,v.product_id, v.measurement,o.*,o.total as order_total,o.wallet_balance,oi.active_status as oi_active_status,u.email,u.name as uname,u.country_code,p.name as pname,(SELECT short_code FROM unit un where un.id=v.measurement_unit_id)as mesurement_unit_name 
        FROM `order_items` oi
                ...
                <div class="box-body">
                    <table class="table table-bordered">
                        ...
                        <tr>
                            ...
                            <td><?= ($fn->get_customer_privacy($seller_id)) ? $res[0]['email'] : str_repeat("*", strlen($res[0]['email']) - 13) . substr($res[0]['email'], -13); ?></td>
                        </tr>
                        <tr>
                            <th style="width: 10px">Contact</th>
                            <td><?= ($fn->get_customer_privacy($seller_id)) ? $res[0]['mobile'] : str_repeat("*", strlen($res[0]['mobile']) - 3) . substr($res[0]['mobile'], -3); ?></td>
                        </tr>
                        <tr>
                        ...
                        <tr>
                            ...
                            ?>
                            <td><?= (!empty($res_pincodes)) ? $res_pincodes[0]['pincode'] . " - " . $res_pincodes[0]['city'] : "" ?></td>
                        </tr>
                                    ...
                                    foreach ($items as $item) { ?>
                                        <div class="card col-md-3">
                                            <div class="card-body">
                                                ...
                                                ?>
                                                <select name="status" id="status" class="form-control update_order_item_status">
...
<script>
    ...
    $(document).on('change', '.update_order_item_status', function(e) {
        e.preventDefault();
        if (allowed == 0) {
        ...
        var status1 = $("option:selected", this).val();
        var id = $('#order_id').val();
        var item_id = $('option:selected', this).data('value1');
        // alert("STATUS : " + status1  + " ITEM ID: " + item_id);

        var dataString = 'update_order_status=1&order_id=' + id + '&status=' + status1 + '&order_item_id=' + item_id + '&ajaxCall=1';
